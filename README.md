...
### 8. Настройка (добавление) спонсорского канала
Чтобы пользователи вашего прокси видели ваш канал в закрепе (Pinned Chat):

1. В боте [@MTProxybot](https://t.me/MTProxybot) отправьте команду `/myproxies`.
2. Нажмите на кнопку вашего прокси (IP:PORT).
3. Нажмите кнопку **«Add promotion»**.
4. Отправьте боту юзернейм вашего публичного канала в формате `@username` или `t.me/username`.
5. **Важно:** Бот подтвердит установку. Изменения вступят в силу в течение 1 часа (это время необходимо для кэширования настроек на стороне серверов Telegram).
   * *Примечание:* Убедитесь, что ваш канал публичный. С приватными каналами эта функция не работает.
   
### 9. Создание ссылки для подключения.

Формат ссылки:
```bash
tg://proxy?server=IP_ВАШЕГО_СЕРВЕРА&port=443&secret=ВАШ_ДЛИННЫЙ_HEX_СЕКРЕТ
```
Пример:
```bash
tg://proxy?server=131.250.347.834&port=443&secret=eebd64616849770f06c008dc1d2ae5bed6766b2e7275
```
